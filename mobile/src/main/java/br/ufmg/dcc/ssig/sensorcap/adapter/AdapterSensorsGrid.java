...
public class AdapterSensorsGrid extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private static final Map<Integer, String> reportingModes;
    static {
        @SuppressLint("UseSparseArrays") Map<Integer, String> map = new HashMap<>();
        map.put(Sensor.REPORTING_MODE_CONTINUOUS, "Continuous");
        map.put(Sensor.REPORTING_MODE_ON_CHANGE, "On Change");
        map.put(Sensor.REPORTING_MODE_ONE_SHOT, "One Shot");
        map.put(Sensor.REPORTING_MODE_SPECIAL_TRIGGER, "Special Trigger");
        reportingModes = map;
    }
    ...
    private MaterialDialog makeSensorDetailsDialog(final SensorsGridItem sensorsGridItem){
        ...
        TextView textSensorReportMode = viewSensorDetails.findViewById(R.id.sensor_report_mode);
        ...
        textSensorReportMode.setText(AdapterSensorsGrid.reportingModes.get(sensorsGridItem.getReportingMode()));
        ...
        if ((sensorsGridItem.getReportingMode() == Sensor.REPORTING_MODE_CONTINUOUS || sensorsGridItem.getReportingMode() == Sensor.REPORTING_MODE_ON_CHANGE) && (sensorsGridItem.getMaxFrequency() > sensorsGridItem.getMinFrequency()) ){
            ...
            if(!sensorsGridItem.isFrequencyEditable()){
                seekSensorFrequency.forbidUserToSeek(true);
                viewSensorDetails.findViewById(R.id.sensor_frequency_is_global_text).setVisibility(View.VISIBLE);
            }
